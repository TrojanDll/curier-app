...
- [x] §14 Этап 3 — Интеграция Admin ↔ Backend
...
Назначение — **взвешенный многокритериальный подбор** (эволюция исходного
«дольше всех на базе»). Полное описание с формулой нормализации, краевыми
случаями и числовым примером — `docs/assignment.md`.
...
3. Для каждого свободного курьера посчитать **взвешенную оценку** `score = Σ wᵢ·factorᵢ` по четырём факторам, нормированным min-max по пулу кандидатов в `[0..1]`:
   - `idle` — как давно на базе (`now − last_returned_at`, fallback `created_at`); больше = лучше (воспроизводит старую логику «дольше всех отдыхает»);
   - `speed` — средняя длительность доставки `delivered_at − assigned_at` по истории; меньше = лучше (нет истории → нейтраль 0.5);
   - `fairness` — число заказов за смену (с 00:00 UTC); меньше = лучше;
   - `experience` — число выполненных доставок за всё время; больше = лучше.
   Веса зависят от **приоритета заказа** (`OrderPriority` = `low`/`normal`/`high`): для `high` доминируют `speed`+`experience` (срочное — быстрому и опытному), для `normal`/`low` — `idle`+`fairness` (равномерная ротация). Старый алгоритм — частный случай (вес `idle=1`). Назначить курьера с максимальной оценкой (тай-брейк — кто раньше нанят).
4. Если свободных нет → заказ создаётся со статусом `new` без `courier_id`. Когда любой курьер возвращается (`status=returned`), запускается **«разгребатель очереди»** — берёт заказ с **наивысшим приоритетом** (`priority DESC, created_at ASC` — не просто самый старый) и назначает этому курьеру.
...
Вся фаза «выбор → назначение» выполняется под Postgres advisory-lock + CAS на строке заказа (защита от гонок при параллельных триггерах). Веса вынесены константами в `scoring.ts` (чистый модуль, покрыт unit-тестами); перенос их в `AppSettings` для настройки админом — в списке развития (`docs/assignment.md`).
...
- [x] **3.7** Socket.IO клиент в admin, обновление таблиц realtime
- [x] **3.8** Загрузка и просмотр фото доставки в drawer-е заказа
...
- [x] **6.1** `docs/DEPLOYMENT.md` — пошаговая инструкция развёртывания на русском
- [x] **6.2** `docs/ADMIN_USER_MANUAL.md` — мануал админа _(текст без скриншотов — UI стабилизировался, скриншоты добавит владелец дистрибутива в свой fork если нужно)_
- [x] **6.3** `docs/COURIER_USER_MANUAL.md` — мануал курьера _(аналогично §6.2: текст без скриншотов APK)_
- [x] **6.4** Корневой `README.md` — что это, как запустить, ссылки
- [x] **6.5** Обновить `architecture_design.md` и `dependencies.md` под новый стек _(deprecation banner с указателями на актуальные v2-источники; legacy-body оставлен как исторический артефакт)_
- [x] **6.6** Подготовить релизный артефакт: git tag + zip-архив + APK файл _(локальный git tag v2.0.0 создан; финальный архив (`git archive`) и signed APK (см. android-release-build.md) собирает владелец перед раздачей — у нас нет его keystore)_
