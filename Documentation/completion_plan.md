...
- [ ] §15a — Подготовительные шаги
- [ ] §14 Этап 1 — Admin-панель на моках
...
- [ ] **1.9** TypeScript строгая типизация + ESLint конфиг
...
- [ ] **15a.5** Согласовать с пользователем момент переноса Android-проекта в `/android` (ломает пути в Android Studio — отдельным коммитом) (in progress: ожидание подтверждения пользователя)
- [ ] **15a.6** Выполнить перенос Android в `/android` (после получения согласия — пункт можно отложить, если пользователь хочет позже)
- [ ] **15a.7** Закоммитить подготовительные изменения сообщением `chore: prepare repo for v2 scope`
- [ ] **15a.8** Отметить этап 15a в §0 как завершённый и перейти к Этапу 1.1
